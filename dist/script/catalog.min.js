...
const createCatalog_Cards_formatCartPrice = stringPrice => {
...
const createCatalog_Cards_createCart = ({
  ...
  if (String(priceFinal).length >= 4) {
    priceFinal = createCatalog_Cards_formatCartPrice(String(priceFinal));
  }
  if (String(priceStart).length >= 4) {
    priceStart = createCatalog_Cards_formatCartPrice(String(priceStart));
  }
...
const fetchCard_getHashFromURL = () => {
  const url = decodeURI(document.location.hash);
  return url.slice(1);
};
const renderCatalog = async () => {
  const catalogName = fetchCard_getHashFromURL();
...
const renderPageCard = async () => {
  const [category, goodID] = fetchCard_getHashFromURL().split('#');
...
;// CONCATENATED MODULE: ./src/script/modules/openCatalogPage.js

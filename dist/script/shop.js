...
const createCartListPrice = (cartListItem, price, discount, count) => {
  if (count === '-') count = 0;
  const priceStart = commonFunction_formatPrice(price * count);
  const priceFinal = commonFunction_formatPrice(commonFunction_getPriceFinal(price * count, discount));
...
const renderShopPage = () => {
  ...
  if (cart) {
    cart.forEach(async item => {
      ...
      const data = await fetchCard_fetchGoods(urlParam);
...
const changeTotalSum = () => {
  ...
  localStorageCart.forEach(({
    ...
    priceFinal += commonFunction_getPriceFinal(price, discount) * count;
  });
  document.querySelector('.total__sumFinal').textContent = commonFunction_formatPrice(priceFinal) + ' ₽';
  ...
  CARTLIST.addEventListener('click', ({
    ...
    if (target.classList.contains('cart-list__cart')) {
      ...
      if (currentCheckbox.checked) {
        const localStorageCart = localStorageCart_getLocalStorage();
        const id = getIdFromItem(currentRow);
        const index = commonFunction_getIndexGoodInLocalStorage(localStorageCart, id);

...
const renderCard = ({
  ...
  const priceFinal = commonFunction_formatPrice(Math.round(price * count * (100 - discount) / 100));
...
const renderShopPage = () => {
  ...
  cartItems.forEach(async item => {
    ...
    const data = await fetchCard_fetchGoods(url + item.id);
...
const makeAllSum = () => {
  ...
  totalSumFinal.textContent = commonFunction_formatPrice(String(sum - saleSum)) + ' ₽';
  spanCount.textContent = count;
  totalSale.textContent = commonFunction_formatPrice(String(saleSum)) + ' ₽';
  totalSumStart.textContent = commonFunction_formatPrice(String(sum)) + ' ₽';
  console.warn(count);
  console.warn(sum);
  ...
  CARTLIST.addEventListener('click', async ({
    ...
    if (target.classList.contains('count__btn_plus')) {
      ...
      if (dataGood.discount) {
        ...
        console.log(price);
        const priceStart = commonFunction_formatPrice(dataGood.price * count);
        const priceFinal = commonFunction_formatPrice(Math.round(dataGood.price * count * (100 - dataGood.discount) / 100));
        fieldPriceFinally.textContent = priceFinal;
        fieldPriceStart.textContent = priceStart;
      } else {
        const price = dataGood.price;
        console.log(price);
        const priceFinal = commonFunction_formatPrice(dataGood.price * count);
        fieldPriceFinally.textContent = priceFinal;
      }

...
const commonFunction_getIndexGoodInLocalStorage = (localStorageCart, id) => localStorageCart.findIndex(item => item.id === id);
...
const createCart = ({
  ...
  cart.insertAdjacentHTML('afterbegin', `     
      ...
      `);
  const priceFinal = commonFunction_getPriceFinal(priceStart, discount);
  const formatPriceFinal = commonFunction_formatPrice(priceFinal);
  const formatPriceStart = commonFunction_formatPrice(priceStart);
...
const createElements_createCartListPrice = (cartListItem, price, discount, count) => {
  if (count === '-') count = 0;
...
const createElements_createCartListItem = ({
  ...
  `);
  createElements_createCartListPrice(cartListItem, price, discount, count);
  return cartListItem;
};
...
const renderPageCard = async () => {
  const [category, goodID] = commonFunction_getHashFromURL('category', 'id');
  renderBreadCrumb(category);
  const urlPageParams = `/goods/${goodID}`;
  const dataGood = await fetchCard_fetchGoods(urlPageParams);
  const {
    titlePage,
    wrapCard,
    titleDescript,
  ...
  const dataRecommend = await fetchCard_fetchGoods(urlCatalogParams);
  const {
    titleRecommend,
    wrapRecommend
...
const render_showCountGoodInCart = flag => {
...
;// CONCATENATED MODULE: ./src/script/card.js
